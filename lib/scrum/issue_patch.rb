  ...
  module IssuePatch
    def self.included(base)
      base.class_eval do
        ...
        # Modified TLE
        def story_points
          if (!((custom_field_id = Scrum::Setting.story_points_custom_field_id).nil?) and
             !((custom_value = self.custom_value_for(custom_field_id)).nil?) and
             !((value = custom_value.value).blank?))
            if has_story_points_task?
              # Replace invalid float number separator (i.e. 0,5) with valid separator (i.e. 0.5)
              value.gsub(',', '.').to_f
            elsif has_story_points?
              value = 0
              tasks = children.select{|issue| (issue.visible?)}
              tasks.each do |task|
                if (!((custom_value = task.custom_value_for(custom_field_id)).nil?) and
                    !((v = custom_value.value).blank?))
                  value = value + v.to_i
                end
              end
              value.to_f
        ...
        def story_points=(value)
          if has_story_points? or has_story_points_task? and
             !((custom_field_id = Scrum::Setting.story_points_custom_field_id).nil?) and
             !((custom_value = self.custom_value_for(custom_field_id)).nil?) and
        ...
        def remaining_story_points
          if !((custom_field_id = Scrum::Setting.story_points_custom_field_id).nil?) and has_remaining_story_points?
            if self.is_task?
              return pending_efforts.last.effort
            end
            tasks = self.children.select{|issue| (issue.visible? and !(issue.status.is_closed? or issue.status.name == "Résolu"))}
            value = 0
            tasks.each do |task|
              if (!((custom_value = task.custom_value_for(custom_field_id)).nil?) and
                  !((v = custom_value.value).blank?))
